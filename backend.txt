...
function loadRuntimeConfig_() {
  ...
  return {
    ...
    ENDPOINT: String(
      p.getProperty('ARK_ENDPOINT') || 'deepseek-v3-2-251201',
    ).trim(),
    API_URL: String(
    ...
    ALLOWED_DOMAIN: String(
      p.getProperty('ALLOWED_DOMAIN') || '@terabox.jp',
    ).trim(),
...
var CONFIG = {};
['API_KEY', 'ENDPOINT', 'API_URL', 'ALLOWED_DOMAIN', 'DEFAULT_TZ'].forEach(
  function (k) {
...
function installScriptPropertiesFromLocal(input) {
  ...
  if (Object.keys(flat).length === 0) {
    throw new Error(
      'installScriptPropertiesFromLocal: 書き込むキーがありません',
    );
  }
...
function checkAccess() {
  try {
    ...
    try {
      ...
      console.warn(
        'getActiveUser() failed (expected when Execute-as=Me): ' + e.toString(),
      );
    ...
    try {
      const config =
        SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Prompt_Config');
      if (config) {
        const whitelist = String(config.getRange('B8').getValue() || '').trim();
        if (whitelist) {
          const allowed = whitelist.split(',').map(function (e) {
          ...
          if (allowed.indexOf(email.toLowerCase()) === -1) {
            return {
              ...
              reason:
                ...
                email +
                ') is not in the authorized list.',
            };
          }
        }
      }
    } catch (e) {
      /* Prompt_Config 不存在时跳过白名单校验 */
    }
  ...
  try {
    ...
    if (name === 'Review_Buffer') {
      ...
      sheet
        .getRange('B3')
        .setValue(
          "You are a lead analyst. Analyze this IT staffing inquiry and return JSON {type:'SES'|'Solution', action:'Go'|'Skip', reason:'...'}.",
        );
...
function getDashboardData() {
  ...
  try {
    ...
    try {
      ...
      result.syncInterval = configSheet
        ? parseInt(String(configSheet.getRange('B9').getValue() || '60'), 10) ||
          60
        : 60;
      result.lastSyncTime = '';
      if (logs && logs.getLastRow() > 1) {
        ...
        for (var li = logData.length - 1; li >= 1; li--) {
          ...
          if (logSubject.indexOf('Sync complete') >= 0) {
            try {
              result.lastSyncTime = Utilities.formatDate(
                new Date(logData[li][0]),
                CONFIG.DEFAULT_TZ,
                'yyyy/MM/dd HH:mm:ss',
              );
            } catch (e) {
              result.lastSyncTime = String(logData[li][0]);
            }
            break;
          }
        }
      ...
      if (!result.lastSyncTime && configSheet) {
        result.lastSyncTime = String(
          configSheet.getRange('B10').getValue() || '',
        );
      }
    } catch (e) {}
...
function getStatisticsData() {
  ...
  try {
    ...
    result.byCategory = Object.keys(catMap)
      .map(function (k) {
    ...
    result.byIndustry = Object.keys(indMap)
      .map(function (k) {
        return { label: k, count: indMap[k] };
      })
      .sort(function (a, b) {
        return b.count - a.count;
      })
    ...
    var TOPIC_KEYWORDS = [
      {
        label: 'ERPシステム',
        keys: ['ERP', 'SAP', 'Oracle', '会計', '販売管理'],
      },
      {
        label: 'AI・機械学習',
        keys: ['AI', '機械学習', 'ML', 'ChatGPT', 'LLM', '生成AI'],
      },
      { label: 'Webアプリ', keys: ['Web', 'EC', 'ポータル', 'CMS', 'PWA'] },
      {
        label: 'RPA・自動化',
        keys: ['RPA', '自動化', 'ロボット', 'Power Automate'],
      },
      {
        label: 'データ分析',
        keys: ['データ分析', 'BI', 'ダッシュボード', 'Tableau', 'Power BI'],
      },
      {
        label: 'クラウド移行',
        keys: ['AWS', 'Azure', 'GCP', 'クラウド', '移行'],
      },
      {
        label: 'セキュリティ',
        keys: ['セキュリティ', '認証', 'SSO', 'ゼロトラスト'],
      },
      {
        label: '基幹システム',
        keys: ['基幹', 'CRM', 'SFA', 'Salesforce', 'ServiceNow'],
      },
      {
        label: 'モバイルアプリ',
        keys: ['iOS', 'Android', 'スマホ', 'アプリ', 'Flutter'],
      },
      {
        label: 'インフラ',
        keys: ['インフラ', 'サーバ', 'ネットワーク', 'オンプレ'],
      },
    ];
    rows.forEach(function (r) {
      var v = String(r[19] || r[20] || '').trim(); // T列(19) or U列(20)構築方法
      if (!v) {
        topicMap['その他'] = (topicMap['その他'] || 0) + 1;
        return;
      }
      var matched = false;
    ...
    result.byAITopic = Object.keys(topicMap)
      .map(function (k) {
        return { label: k, count: topicMap[k] };
      })
      .sort(function (a, b) {
        return b.count - a.count;
      })
    ...
    console.error('getStatisticsData error: ' + e);
  }
  return result;
}
...
function saveOrUpdateLead(data, status) {
  ...
  // 辅助：有值才写（不覆盖已有内容为空）
  const write = function (r, col, val) {
    if (val) sheet.getRange(r, col).setValue(val);
  };
...
function parseBodyToStructured(rawBody, cachedSystemMsg) {
  ...
  if (!systemMsg) {
    try {
      ...
      console.warn('Prompt_Config B2 read failed: ' + e);
    }
  }
  ...
  if (!raw) {
    console.error(
      'parseBodyToStructured: JSON parse failed. Response: ' +
        res.slice(0, 300),
    );
    if (regexCaseId || regexCorp) {
      return { 案件ID: regexCaseId, 法人名: regexCorp, 分类建议: 'Solution' };
...
function submitConsultantDecision(d) {
  ...
  try {
    ...
    if (cachedJson) {
      ...
      console.log(
        'Using cached ParsedJSON, keys=' + Object.keys(structured).length,
      );
    }
  } catch (e) {
  ...
  const countNonEmpty = function (obj) {
    return Object.keys(obj).filter(function (k) {
      return obj[k] && String(obj[k]).trim() !== '';
    }).length;
  };
  if (countNonEmpty(structured) < 5 && rawBody) {
    console.log(
      'ParsedJSON has too few non-empty fields (nonempty=' +
        countNonEmpty(structured) +
        '), calling AI full parse...',
    );
    try {
      ...
      if (savedCorp && savedCorp !== '[UNIDENTIFIED]')
        structured['法人名'] = savedCorp;
  ...
  if (!structured['案件ID'] || structured['案件ID'] === '')
    structured['案件ID'] = caseId;
  if (
    !structured['法人名'] ||
    structured['法人名'] === '' ||
    structured['法人名'] === '[UNIDENTIFIED]'
  ) {
    structured['法人名'] =
      corp && corp !== '[UNIDENTIFIED]' ? corp : structured['法人名'] || '';
  }

  // 合并顾问裁决
  structured['分类建议'] =
    d.type || structured['分类建議'] || structured['所属部門'] || 'SES';
  ...
  // 顾问 Notes 追加到 相談内容
  const verdictNote =
    '[VERDICT:' + d.decision.toUpperCase() + '] ' + (d.reason || '');
  structured['相談内容'] =
    (structured['相談内容'] ? structured['相談内容'] + '\n\n' : '') +
    verdictNote;
  ...
  for (let i = 1; i < allData.length; i++) {
    ...
    const isMatch =
      (caseId && caseId !== 'NEW' && caseId !== '' && rowCaseId === caseId) ||
  ...
  try {
    ...
    console.warn('saveFeedbackLog failed: ' + eLog);
  }
...
function saveMeetingRecord(d) {
  ...
  const rows =
    sheet.getLastRow() > 1 ? sheet.getDataRange().getValues().slice(1) : [];
  ...
  if (decisionRaw.indexOf('go') >= 0 || decisionRaw.indexOf('bid') >= 0) {
    ...
    decisionRaw.indexOf('skip') >= 0 ||
    decisionRaw.indexOf('ng') >= 0
  ) {
...
function backfillMeetingStatus(forceOverwrite) {
  ...
  var ciSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Case_infor');
  if (!ciSheet)
    return {
      updated: 0,
      skipped: 0,
      notFound: 0,
      error: 'Case_infor not found',
    };
  ...
  for (var m = 0; m < mrRows.length; m++) {
    ...
    if (mrDecision.indexOf('go') >= 0 || mrDecision.indexOf('bid') >= 0) {
      ...
      mrDecision.indexOf('skip') >= 0 ||
      mrDecision.indexOf('ng') >= 0
    ) {
      newStatus = 'Reviewed (Skip)';
    }
    if (!newStatus) {
    ...
    // Case_infor の対応行を探す
    var targetRow =
      mrCaseId && ciIndexById[mrCaseId]
        ? ciIndexById[mrCaseId]
        : mrCorp && ciIndexByCorp[mrCorp]
          ? ciIndexByCorp[mrCorp]
          : 0;
    if (!targetRow) {
      notFound++;
      continue;
    ...
    if (existingStatus && !forceOverwrite) {
      skipped++;
      continue;
    }
...
function getCaseTodos(caseId) {
  ...
  rows.forEach(function (r, idx) {
    ...
    try {
      createdStr = r[7]
        ? Utilities.formatDate(new Date(r[7]), CONFIG.DEFAULT_TZ, 'yyyy/MM/dd')
        : '';
    } catch (e) {}
    result.push({
...
function generateOutreachEmail(caseId, action, templateType) {
  try {
    ...
    if (!sheet || sheet.getLastRow() < 2) {
      return {
        subject: '',
        body: '案件データが見つかりません (Case_infor が空)。',
      };
    }
    const rows = sheet.getDataRange().getValues();
    var caseRow = null;
    ...
    if (!caseRow) {
      return {
        subject: '',
        body: '案件ID「' + caseId + '」が Case_infor に見つかりません。',
      };
    }
    ...
    try {
      ...
      console.warn('Review_Buffer read failed: ' + e);
    }
    ...
    try {
      ...
      console.warn('Meeting_Records read failed: ' + e);
    }
    ...
    try {
      ...
      if (fl && fl.getLastRow() > 1) {
        ...
        flRows.forEach(function (r) {
          ...
          if (note)
            comments.push('・[' + name + ' / ' + decision + '] ' + note);
        });
        advisorComments = comments.join('\n');
      }
    } catch (e) {
      console.warn('Feedback_Log read failed: ' + e);
    }
    ...
    var userMsg =
      ...
      '\n' +
      '- 相談背景: ' +
      background.slice(0, 200) +
      '\n' +
      '- 現状課題: ' +
      challenge.slice(0, 200) +
      '\n' +
      '- 主要要件: ' +
      coreReq.slice(0, 200) +
      '\n' +
    ...
    if (!aiRaw)
      return {
        subject: '',
        body: 'AI生成に失敗しました。API接続を確認してください。',
      };
    ...
    return {
      subject: '【テラボックス】ご案件についてのご確認 - ' + corp + '様',
      body: aiRaw,
    };
  } catch (e) {
...
function parseContentWithAI(fullText) {
  ...
  try {
    ...
    console.error('parseContentWithAI→parseBodyToStructured failed: ' + e);
  }
  // 兜底：シンプルな4フィールド抽出
  const systemMsg =
    'Extract JSON only: {"案件ID":"","法人名":"","相談内容":"","分类建议":"SES"}. 案件ID is a 7-digit number found in subject or body. 法人名 is the company name of the sender or requester.';
  const res = callAIText(
...
function gmailAgentRunner(options) {
  ...
  try {
    ...
    const targetEmail =
      (config.getRange('B4').getValue() || '').toString().trim() ||
      'rc_support@frontier-gr.jp';
    ...
    _log(
      `🔍 Scan started`,
      'RUNNING',
      `after:${afterStr} includeRead=${!!options.includeRead} query:${query}`,
    );
    ...
    for (let i = 0; i < threads.length; i++) {
      ...
      if (!msg) {
        skipped++;
        _log(
          `[${i + 1}/${total}] ⏭ SKIP (dup-thread-all-msgs)`,
          'SKIP',
          msgs[0].getSubject(),
        );
        continue;
      ...
      const subjKey =
        'subj:' + subject.trim().replace(/\s+/g, ' ').toLowerCase();
      ...
      const spamCheck = checkSpamFilter(
        '',
        subject,
      ...
      if (spamCheck.skip) {
        skipped++;
        _log(
          `[${i + 1}/${total}] 🚫 SKIP (Spam_Filter)`,
          'SKIP',
          spamCheck.reason + ' | ' + subject,
        );
        continue;
      }
      ...
      const corp =
        aiData && aiData['法人名'] && aiData['法人名'] !== ''
          ? aiData['法人名']
          : '[UNIDENTIFIED]';
      const caseId =
        aiData && aiData['案件ID'] ? String(aiData['案件ID']).trim() : '';
      ...
      if (
        ...
        caseId.length >= 5 &&
        /^\d+$/.test(caseId) &&
        dupSet['caseid:' + caseId]
      ) {
        skipped++;
        _log(
          `[${i + 1}/${total}] ⏭ SKIP (dup-caseid=${caseId})`,
          'SKIP',
          subject,
        );
        continue;
      ...
      try {
        var fullParsed =
          aiData && Object.keys(aiData).length >= 3 ? aiData : {};
        parsedJson =
          Object.keys(fullParsed).length > 0 ? JSON.stringify(fullParsed) : '';
        _log(
          `[${i + 1}/${total}] 📋 Full parse OK`,
          'AI',
    ...
    try {
      const syncTimeStr = Utilities.formatDate(
        new Date(),
        CONFIG.DEFAULT_TZ,
        'yyyy/MM/dd HH:mm',
      );
      ...
      console.warn('B10 write failed: ' + e);
    }
...
function getLastSyncTime() {
  try {
    var v = String(
      getSheetSafe('Prompt_Config').getRange('B10').getValue() || '',
    ).trim();
    return v || null;
  } catch (e) {
    return null;
...
function getSyncInterval() {
  try {
    var v = parseInt(
      String(getSheetSafe('Prompt_Config').getRange('B9').getValue() || '60'),
      10,
    );
...
function setSyncIntervalAndRebuildTrigger(minutes) {
  try {
    ...
    console.log(
      'setSyncIntervalAndRebuildTrigger: new trigger set to ' +
        minutes +
        ' min',
    );
    return {
      ok: true,
      interval: minutes,
      message: '自動同期を ' + minutes + ' 分ごとに設定しました',
    };
  } catch (e) {
...
function callAI(q, s) {
  ...
  try {
    return (
      JSON.parse(UrlFetchApp.fetch(CONFIG.API_URL, options).getContentText())
...
function callAIText(q, s) {
  ...
  try {
    ...
    if (!content) {
      console.error(
        'callAIText: empty content. HTTP response=' + raw.slice(0, 400),
      );
    } else {
...
function debugParseLatestBufferRow() {
  const sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Review_Buffer');
  if (!sheet || sheet.getLastRow() < 2) {
  ...
  console.log(
    '=== DEBUG: B2 prompt length=' + b2.length + ' preview=' + b2.slice(0, 100),
  );
  ...
  console.log(
    '=== DEBUG: AI raw response=' + (res ? res.slice(0, 500) : 'NULL'),
  );

  // JSON parse
  const parsed = safeJsonParse(res);
  console.log(
    '=== DEBUG: parsed keys=' + (parsed ? Object.keys(parsed).length : 0),
  );
  if (parsed) {
    const nonEmpty = Object.keys(parsed).filter(function (k) {
      return parsed[k] && String(parsed[k]).trim() !== '';
    });
    console.log(
      '=== DEBUG: non-empty fields (' +
        nonEmpty.length +
        ')=' +
        nonEmpty.join(', '),
    );
    ...
    console.log(
      '=== DEBUG: 相談内容=' + (parsed['相談内容'] || '').slice(0, 100),
    );
  }
}
...
function getReviewBufferList() {
  try {
    ...
    try {
      ...
      console.warn('Case_infor JOIN error: ' + e);
    }
    ...
    try {
      ...
      console.warn('Todo JOIN error: ' + e);
    }

    const list = [];
    for (let i = 1; i < data.length; i++) {
      const rowStatus = String(data[i][10] || '')
        .trim()
        .toUpperCase();
      ...
      var syncDate = null;
      try {
        mailDate = new Date(data[i][0]);
        if (isNaN(mailDate.getTime())) mailDate = null;
      } catch (e) {
        mailDate = null;
      }
      try {
        syncDate = new Date(data[i][11]);
        if (isNaN(syncDate.getTime())) syncDate = null;
      } catch (e) {
        syncDate = null;
      }
      ...
      try {
        dStr = rawDate
          ? Utilities.formatDate(rawDate, CONFIG.DEFAULT_TZ, 'yyyy/MM/dd HH:mm')
          : 'Unknown';
      } catch (e) {
        dStr = 'Unknown';
      }
      ...
      if (!corpVal && parsedFields)
        corpVal = String(parsedFields['法人名'] || '').trim();
      if (!titleVal && parsedFields)
        titleVal = String(parsedFields['相談内容'] || '').trim();
      ...
      list.push({
        ...
        rawMailDate: rawMailDate,
        rawSyncDate: rawSyncDate,
...
function runAIPreEvaluation() {
  ...
  try {
    ...
    console.warn('Knowledge injection failed: ' + eKnow);
  }
    ...
    if (String(data[i][10]).toUpperCase() === 'PENDING' && !data[i][7]) {
      const res = callAI(
        systemPrompt +
          '\n\nContent: ' +
          data[i][4] +
          '\nReturn JSON {type, action, reason}（reasonは日本語で記述）',
        'Lead Scorer',
      );
      ...
      if (dec)
        buffer
          .getRange(i + 1, 8, 1, 3)
          .setValues([
            [dec.type || 'Solution', dec.action || 'Skip', dec.reason || 'Ok'],
          ]);
    }
  }
}
...
function reparseBufferUnidentified() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const needReparse =
      corp === '[UNIDENTIFIED]' || !caseId || nonEmptyCount < 5;
    ...
    try {
      ...
      const newCorp =
        parsed['法人名'] && parsed['法人名'] !== '' ? parsed['法人名'] : corp;
      const newCaseId = parsed['案件ID']
        ? String(parsed['案件ID']).trim()
        : caseId;

      const rowNum = i + 1;
      if (newCaseId) sheet.getRange(rowNum, 2).setValue(newCaseId);
      if (newCorp && newCorp !== '[UNIDENTIFIED]')
        sheet.getRange(rowNum, 3).setValue(newCorp);
      sheet.getRange(rowNum, 13).setValue(JSON.stringify(parsed));

      console.log(
        `reparseBuffer row${rowNum}: corp="${newCorp}" id="${newCaseId}" fields=${Object.keys(parsed).length}`,
      );
      reparsed++;
    } catch (e) {
...
function getKnowledgeData() {
  ...
  try {
    ...
    console.warn('getKnowledgeData feedbacks: ' + e);
  }
  try {
    const pdSheet = ss.getSheetByName('Project_Discussion');
    if (pdSheet && pdSheet.getLastRow() > 1) {
    ...
    console.warn('getKnowledgeData discussions: ' + e);
  }
  return result;
}
...
function addProjectDiscussion(d) {
  if (!d || !d.experience)
    return { status: 'ERROR', msg: 'experience required' };
...
function deleteProjectDiscussion(rowNum) {
  try {
    ...
    if (rowNum < 2 || rowNum > sheet.getLastRow())
      return { status: 'ERROR', msg: 'Invalid row' };
...
function chatWithCaseAI(userMsg, history, scope) {
      ...
      if (sheet && sheet.getLastRow() > 1) {
        ...
        var rows = needCaseSingle
          ? data.filter(function (r) {
              return String(r[0]).trim() === scope.replace('case:', '').trim();
            })
          : data.slice(-20);
        if (rows.length > 0) {
          ctx += '\n\n# 【案件アーカイブ（Case_infor）】\n';
          rows.forEach(function (r) {
            ...
            if (needCaseSingle) {
              if (r[13])
                ctx += '   法人概要: ' + String(r[13]).slice(0, 300) + '\n';
              if (r[14])
                ctx += '   相談内容: ' + String(r[14]).slice(0, 400) + '\n';
              if (r[15])
                ctx += '   相談背景: ' + String(r[15]).slice(0, 300) + '\n';
              if (r[16])
                ctx += '   現状課題: ' + String(r[16]).slice(0, 300) + '\n';
              if (r[22])
                ctx += '   核心ニーズ: ' + String(r[22]).slice(0, 200) + '\n';
            }
          });
        }
      }
    } catch (e) {
      console.warn('ctx Case_infor: ' + e);
    }
  }
        ...
        if (bufData.length > 0) {
          ...
          bufData.forEach(function (r) {
            ctx +=
              ...
              '\n';
            if (r[6])
              ctx += '   本文抜粋: ' + String(r[6]).slice(0, 300) + '\n';
          });
        }
      }
    } catch (e) {
      console.warn('ctx Review_Buffer: ' + e);
    }
  }
  ...
  if (scope === 'knowledge' || scope === 'full') {
    try {
      ...
      console.warn('ctx Project_Discussion: ' + e);
    }

    // ── ④ Feedback_Log（顧問決定履歴）──────────────────
    try {
      var flSheet =
        SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Feedback_Log');
      if (flSheet && flSheet.getLastRow() > 1) {
        var flData = flSheet
          .getDataRange()
          .getValues()
        ...
        if (flData.length > 0) {
          ...
          flData.forEach(function (r) {
            ctx +=
              ...
              '\n';
            if (r[4])
              ctx += '   判断理由: ' + String(r[4]).slice(0, 150) + '\n';
          });
        }
      }
    } catch (e) {
      console.warn('ctx Feedback_Log: ' + e);
    }
  }
...
function getCaseListForChat() {
  ...
  try {
    ...
    console.warn('getCaseListForChat: ' + e);
  }
  return cases;
}
...
function deleteBufferItem(row, reason, memo) {
  try {
    ...
    if (!row || row < 2 || row > lastRow)
      return { status: 'ERROR', msg: '無効な行番号: ' + row };
    ...
    try {
      ...
      if (kwMatch)
        keywords = kwMatch.map(function (s) {
...
function checkSpamFilter(corp, subject, body) {
  try {
    const sfSheet =
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Spam_Filter');
    ...
    for (var i = 0; i < rules.length; i++) {
      var r = rules[i];
      var rCorp = String(r[2] || '')
        .trim()
        .toLowerCase();
      var rDomain = String(r[3] || '')
        .trim()
        .toLowerCase();
      var rSubject = String(r[4] || '')
        .trim()
        .toLowerCase();
      var rKeywords = String(r[5] || '')
        .split(',')
        .map(function (k) {
      ...
      if (rCorp && corpLc.indexOf(rCorp) !== -1) {
        return {
          skip: true,
          reason: 'Spam_Filter: 法人名一致「' + r[2] + '」(' + r[1] + ')',
        };
      }
      // 送信元ドメイン一致
      if (rDomain && bodyLc.indexOf(rDomain) !== -1) {
        return {
          skip: true,
          reason: 'Spam_Filter: ドメイン一致「' + r[3] + '」(' + r[1] + ')',
        };
      }
      // 件名キーワード（登録件名の一部が一致）
    ...
    console.warn('checkSpamFilter: ' + e);
  }
  return { skip: false, reason: '' };
}
...
function buildDuplicateSet() {
  ...
  try {
    ...
    console.warn('buildDuplicateSet Buffer error: ' + e);
  }
  try {
    ...
    console.warn('buildDuplicateSet Case_infor error: ' + e);
  }
  return set;
}
...
function backfillMailDate(dryRun, limit) {
  ...
  for (let i = 1; i < data.length && processed < limit; i++) {
    try {
      ...
      let hasMail = false;
      try {
        hasMail = !!mailCell && new Date(mailCell).getTime() ? true : false;
      } catch (e) {
        hasMail = false;
      }
      if (!hasMail && msgId) {
        ...
        try {
          ...
          if (msg) {
            ...
            if (md && !isNaN(md.getTime())) {
              detail.mailDate = Utilities.formatDate(
                md,
                CONFIG.DEFAULT_TZ,
                'yyyy/MM/dd HH:mm:ss',
              );
...
function diagReviewBuffer() {
  ...
  const sheet = ss.getSheetByName('Review_Buffer');
  if (!sheet) {
    Logger.log('❌ Review_Buffer sheet not found!');
    return;
  }
  const data = sheet.getDataRange().getValues();
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    Logger.log(
      'Row ' +
        ...
        ' | ' +
        'col1(0)=' +
        String(row[0]).slice(0, 20) +
        ' | ' +
        'col2(1)=' +
        String(row[1]).slice(0, 20) +
        ' | ' +
        'col3(2)=' +
        String(row[2]).slice(0, 20) +
        ' | ' +
        'col4(3)=' +
        String(row[3]).slice(0, 30) +
        ' | ' +
        'col5(4)body=' +
        String(row[4]).slice(0, 30) +
        ' | ' +
        'col11(10)status=' +
        String(row[10]) +
        ' | ' +
        'col12(11)sync=' +
        String(row[11]).slice(0, 20) +
        ' | ' +
        'col13(12)json=' +
        String(row[12]).slice(0, 30),
    );
  }
}
  ...
  return {
    ...
    changelog: [
      ...
      },
      {
        version: 'V9.9.0',
        date: '2026-04-11',
        changes: ['AI Chat 統合ナレッジ対話', 'AI日語出力対応'],
      },
    ],
  };
}
...
function saveFeatureRequest(d) {
  if (!d || !d.description)
    return { status: 'ERROR', msg: 'description required' };
...
function updateFeatureRequestStatus(rowNum, status, adminNote) {
  ...
  if (adminNote !== undefined)
    sheet.getRange(rowNum, 7).setValue(String(adminNote || ''));
...
function deduplicateReviewBuffer() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (isDup) {
      ...
      if (caseId && caseId.length >= 5 && /^\d+$/.test(caseId))
        seenCaseId[caseId] = true;
    ...
    if (buf.getLastRow() > 1) {
      ...
      for (var i = 1; i < bData.length; i++) {
        ...
        if (msgId && bSeenMsgId[msgId]) dupReason = 'MsgID重複';
          ...
          dupReason = 'CaseID重複: ' + caseId;
        else if (subj && subj.length > 10 && bSeenSubj[subj])
          dupReason = '件名重複';
        ...
        if (dupReason) {
          result.buffer.push({
            ...
            reason: dupReason,
            status: String(bData[i][10] || ''),
          });
        } else {
          if (msgId) bSeenMsgId[msgId] = true;
          if (caseId && caseId.length >= 5 && /^\d+$/.test(caseId))
            bSeenCaseId[caseId] = true;
          if (subj && subj.length > 10) bSeenSubj[subj] = true;
        }
      }
    }
  } catch (e) {
    console.error('DupReport Buffer: ' + e);
  }
  ...
  try {
    ...
    if (main && main.getLastRow() > 1) {
      ...
      for (var j = 1; j < cData.length; j++) {
        ...
        try {
          dateStr2 = cData[j][2]
            ? Utilities.formatDate(new Date(cData[j][2]), 'Asia/Tokyo', 'MM/dd')
            : '';
        } catch (e) {}
        var dupReason2 = null;

        if (cid && cid.length >= 5 && /^\d+$/.test(cid) && cSeenId[cid])
          dupReason2 = 'CaseID重複: ' + cid;
        else if (cCorp && cCorp !== '[UNIDENTIFIED]' && cSeenCorp[cCorp])
          dupReason2 = '法人名重複: ' + cCorp;

        if (dupReason2) {
          result.caseInfor.push({
            row: j + 1,
            date: dateStr2,
            caseId: cid,
            corp: cCorp,
    ...
    console.error('DupReport CaseInfor: ' + e);
  }
...
function syncToYeeflow() {
  ...
  if (!apiKey)
    throw new Error("Script Property 'YEEFLOW_API_KEY' が設定されていません");
  if (!listId)
    throw new Error("Script Property 'YEEFLOW_LIST_ID' が設定されていません");
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    try {
      ...
      var errMsg =
        'Row' + (i + 1) + ' [' + corp + '] Exception: ' + e.toString();
      result.errors.push(errMsg);
      result.failed++;
  ...
  for (var i = 1; i < rows.length; i++) {
    if (String(rows[i][0]) === String(eventId)) {
      if (data.eventType !== undefined)
        sheet.getRange(i + 1, 3).setValue(data.eventType);
      if (data.eventDate !== undefined)
        sheet.getRange(i + 1, 4).setValue(data.eventDate);
      if (data.title !== undefined)
        sheet.getRange(i + 1, 5).setValue(data.title);
      if (data.description !== undefined)
        sheet.getRange(i + 1, 6).setValue(data.description);
      if (data.attachments !== undefined)
        sheet.getRange(i + 1, 7).setValue(data.attachments);
...
function clearConfigCache() {
  ...
  Logger.log(
    '- API_KEY: ' +
      (config.API_KEY ? config.API_KEY.substring(0, 10) + '...' : 'NULL'),
  );
...
function diagnoseWebAppCall() {
  ...
  try {
    var sheet = getSheetSafe('Case_infor');
...
function getTrackedProjects() {
  ...
  try {
    ...
    Logger.log(
      '[getTrackedProjects] Got sheet, last row: ' + sheet.getLastRow(),
    );
    ...
    for (var i = 1; i < rows.length; i++) {
      ...
      if (
        statusLower.indexOf('hold') >= 0 ||
        statusLower.indexOf('bid') >= 0 ||
        statusLower === 'go'
      ) {
        // 日期对象转换为字符串，避免序列化问题
        var dateStr = '';
    ...
    Logger.log(
      '[getTrackedProjects] Sample project: ' +
